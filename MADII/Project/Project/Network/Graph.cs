...
namespace Project.Network
{
    public class Graph
    {
        ...
        {
            int comp = 0;
            var notVisited = new Dictionary<int ,GraphNode>();
            foreach (var graphNode in NodeList)
            {
                notVisited.Add(graphNode.Id, graphNode);
            }

            void VisitNode(GraphNode node)
            {
                if (notVisited.ContainsKey(node.Id))
                {
                    notVisited.Remove(node.Id);
                }

                foreach (var nodeNeighbor in node.Neighbors)
                {
                    if (notVisited.ContainsKey(nodeNeighbor.Id))
                    {
                        VisitNode(nodeNeighbor);
                    }
                }
            }
            ...
            {
                comp++;
                VisitNode(notVisited.First().Value);
            }
